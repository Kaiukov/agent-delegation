...
Вместо `sleep N` + повторный `cmux read-screen` — подписка на поток событий cmux.
...
- `sleep`-цикл жжёт токены (каждый повторный `cmux read-screen` — отдельный вызов) и время.
...
- `worker-watch.sh` — canonical standby for headless `pi` workers; for interactive cmux event flows, reuse the pattern described in this doc instead of hand-writing the stream pipe.
