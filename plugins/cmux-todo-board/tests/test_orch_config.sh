...
json="$($ORCH_CONFIG --get-profile repo-scout --json)"
assert_json_field "$json" role "repo-scout"
assert_json_field "$json" runtime "pi"
assert_json_field "$json" model "openai-codex/gpt-5.4-mini"
assert_json_field "$json" thinking "low"
assert_json_field "$json" tools "read,bash,grep,find,ls"
...
json="$($ORCH_CONFIG --get-profile backend --json)"
assert_json_field "$json" role "backend"
assert_json_field "$json" runtime "pi"
assert_json_field "$json" model "openai-codex/gpt-5.4-mini"
assert_json_field "$json" thinking "high"
assert_json_field "$json" tools "read,bash,edit,write,grep,find,ls"
assert_json_field "$json" permissions "write"
assert_json_field "$json" launch_mode "via-pi"

echo "=== Test 4: reviewer profile lookup works ==="
json="$($ORCH_CONFIG --get-profile reviewer --json)"
assert_json_field "$json" role "reviewer"
assert_json_field "$json" runtime "pi"
assert_json_field "$json" model "openai-codex/gpt-5.4-mini"
assert_json_field "$json" thinking "med"
assert_json_field "$json" tools "read,bash,grep,find,ls"
assert_json_field "$json" permissions "read-only"
assert_json_field "$json" launch_mode "via-pi"

echo "=== Test 5: output includes required fields ==="
keys_ok="$(echo "$json" | jq -r 'has("role") and has("runtime") and has("model") and has("thinking") and has("tools") and has("permissions") and has("launch_mode")')"
