...
pytestmark = pytest.mark.skipif(shutil.which("codex") is None, reason="codex not installed")
...
def test_codex_runtime_spawn_smoke(tmp_path: Path):
    backend = AgentBackend(tmp_path / "state")
    record = backend.spawn_agent(runtime="codex", prompt="echo smoke", cwd=str(tmp_path), timeout_sec=2)
